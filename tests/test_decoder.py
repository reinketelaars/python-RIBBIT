...
def croakstream():
    croak_data = CROAK_RE.match(ENCODED_CROAK.lstrip()).group(1)
    return base64.b64decode(croak_data)


def test_decoding_valid_CROAK():
    frog_tips = decode_CROAK(croakstream())

    assert 257 in frog_tips
    assert frog_tips[257] == ('FROG IS DESIGNED TO BE THROWN INTO A ROOM TO '
                              'DELIVER A LOUD BANG AND BRIGHT FLASH SUFFICIENT'
                              ' ENOUGH TO TEMPORARILY DISORIENT PERSONNEL IN'
                              ' THE ROOM.')


def test_decoding_invalid_CROAK_raises_exception():
    with pytest.raises(ValueError) as excinfo:
        decode_CROAK('GLAGGLUAGHGGALUGAHGLUAGH')

    assert 'Error during FROG tip parsing' in str(excinfo.value)


ENCODED_CROAK = """
-----BEGIN CROAK-----
MIITvzBIAgFSDENGUk9HIElTIEEgQ0hPS0lORyBIQVpBUkQuIERPIE5PVCBHSVZF
